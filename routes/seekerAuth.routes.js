...
const salt = 10;
const isJobSeekerAuthenticated = require("./../middlewares/isJobSeekerAuthenticated");
...
router.post("/login", async (req, res, next) => {
  try {
    const { email, password } = req.body;
    const foundUser = await JobSeeker.findOne({ email }).select(
      "password email"
    );
    if (!foundUser) {
...
router.get("/me", isJobSeekerAuthenticated, async (req, res, next) => {
  res.json(req.user);
});
